        module parameters
                ...
                integer,save::maxiter,print_iter
        end module parameters
        

! module pour la verification de type argument
        module type
        
                interface verif1
                        subroutine marq98(b,m,ni,v,rl,ier,istop,ca,cb,dd,func_name)
                                integer,intent(in) :: m
                                ...
                                double precision,external::func_name
                        end subroutine marq98

                        subroutine deriva(b,m,v,rl,func_name)
                                integer,intent(in)::m
                        ...
                        subroutine searpas(vw,step,b,bh,m,delta,fim,func_name)
                                integer,intent(in)::m
                                double precision,dimension(m),intent(in)::b
                        ...
                        subroutine valfpa(vw,fi,b,bk,m,delta,func_name)
                                integer,intent(in)::m!
...
!   variables globales
      ...
      double precision,intent(inout)::ca,cb,dd
!   variables locales
      integer::nql,ii,nfmax,idpos,ncount,id,jd,m1,j,i,ij,kkk,k
      double precision,dimension(m*(m+3)/2)::fu
      double precision,dimension(m)::delta,b1,bh
      double precision::da,dm,ga,tr
      double precision::GHG,step,eps  &
      ,vw,fi,z,rl1,th,ep

      double precision::maxt,det
      double precision,external::func_name
! ----------------------------------------------------
...
!     epsd=1.d-3
      ...
      Main:Do
        ...
        call deriva(b,m,v,rl,func_name)
...
!       write(*,*)'ca <epsa',' ca ',ca,' epsa ',epsa
!       write(*,*)'cb <epsb',' cb ',cb,' epsb ',epsb
!       write(*,*)'dd <epsd',' dd ',dd,' epsd ',epsd
...
!      write(6,*) 'loglikelihood not improved '
         ...
         step=dlog(1.5d0)
!      write(*,*) 'searpas'
         call searpas(vw,step,b,bh,m,delta,fi,func_name)
         rl=-fi
         if(rl.eq.-1.D9) then
               istop=4
               goto 110
...
!           ca=ca+dexp(b(i))*(1.d0-dexp(delta(i))**2)
         end do
...
!ajout amadou
                if(ind_hess==1) then
                        ...
                        do j=i,m
                                k = (((j-1)*j)/2) +i
                                hessienne(i,j)= v(k)/(4.d0*b(i)*b(j))
                        end do
                        hessienne(i,i)= hessienne(i,i)+(v(kkk)/(4.d0*b(i)*b(i)*b(i)))
                        end do
                        do i = 2,m
                                do j = 1,i-1
                                        hessienne(i,j)=hessienne(j,i)
                                end do
                        end do    
                end if
...
!
      ...
      do i=1,m
         ...
         fcith(i)=func_name(b,m,i,th,i0,z)
      ...
      Main:do i=1,m
         ...
         do j=1,i
            ...
            temp=func_name(b,m,i,thi,j,thj)
            if(temp.eq.-1.d9) then
               rl=-1.d9
               exit Main
            end if
...
!
      ...
      double precision,external::func_name
       ...
       vlw1=dlog(vw)
       vlw2=vlw1+step
       call valfpa(vlw1,fi1,b,bh,m,delta,func_name)
       call valfpa(vlw2,fi2,b,bh,m,delta,func_name)
...
!
      ...
      call valfpa(vm,fim,b,bh,m,delta,func_name)
