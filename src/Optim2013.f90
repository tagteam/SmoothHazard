...
!   variables locales            
      ...
      double precision::GHG,idmLikelihood,survLikelihood,det,step,eps,vw,fi,maxt, &
      z,rl1,th,ep
	...
	! write(*,*)'b',(b(i),i=1,m),'iteration',ni              
        rl1=rl      
        dd = 0.d0     
        fu=0.D0
        do i=1,m
 ...
 400    do i=1,nfmax+m
        ...
        if (idpos.ne.0) then
           ...
           goto 400
            ...
            ! write(*,*) model 
            if (model.eq.1) then 
               rl=survLikelihood(b1,m,id,z,jd,z)
            else
               if (model.eq.3) then
                  rl=idmLikelihood(b1,m,id,z,jd,z)
               endif
            endif
            if (rl1.lt.rl) then
               if(da.lt.eps) then
...
!------------------------------------------------------------
      ...
      double precision::survLikelihood,idmLikelihood,thn,th,z,vl,temp,thi,thj      
!    
...
!
      ...
      if (model.eq.1) then 
         rl=survLikelihood(b,m,i0,z,i0,z)
      else
         if (model.eq.3) then
            rl=idmLikelihood(b,m,i0,z,i0,z)
         endif
      endif
      ...
      do i=1,m
         ...
         if (model.eq.1) then 
            fcith(i)=survLikelihood(b,m,i,th,i0,z)
         else
            if (model.eq.3) then
               fcith(i)=idmLikelihood(b,m,i,th,i0,z)
            endif
         endif
      ...
      Main:do i=1,m
         ...
         if (model.eq.1) then 
            temp=survLikelihood( b,m, i,thn,i0,z)
         else
            if (model.eq.3) then
               temp=idmLikelihood( b,m, i,thn,i0,z)
            endif
         endif
         ...
         do j=1,i
            ...
            if (model.eq.1) then 
               temp=survLikelihood(b,m,i,thi,j,thj)
            else
               if (model.eq.3) then
                  temp=idmLikelihood(b,m,i,thi,j,thj)
            endif
         endif
            if(temp.eq.-1.d9) then
               rl=-1.d9
...
!------------------------------------------------------------
        ...
        double precision,dimension(m),intent(out)::bk 
        double precision,intent(out)::fi 
	double precision::vw,survLikelihood,idmLikelihood,z	
	integer::i0,i
         ...
         do i=1,m
           ...
           if (model.eq.1) then 
            fi=-survLikelihood(bk,m,i0,z,i0,z)
         else
            if (model.eq.3) then
               fi=-idmLikelihood(bk,m,i0,z,i0,z)
            endif
         endif
