...
#' function.
#' @param s time at which the prediction of lifetime risks
...
lifexpect <- function(x,s,Z01,Z02,Z12,nsim=1000,CI=TRUE,...) {
    if (x$method=="Weib"){
        ...
        if (CI==TRUE) {
            ...
            Xres2 <- apply(Xres1,2,as.numeric) 
            Xres3 <- t(apply(Xres2,1,sort)) 
            iinf=((nsim/100)*2.5) + 1
            isup=(nsim/100)*97.5
            if (iinf != round(iinf)){
            ...
            Xres4 <- cbind(Xres3[,iinf],Xres3[,isup]) 
          ...
          if (CI==TRUE) {
              ...
              Xres1 <- mapply(function(Xtheta01,Xtheta12,Xtheta02,XbZ01,XbZ12,XbZ02) 
                  lifexpect0(s,zi01,nz01,Xtheta01,zi12,nz12,Xtheta12,zi02,nz02,Xtheta02,XbZ01,XbZ12,XbZ02),Xtheta01,Xtheta12,Xtheta02,XbZ01,XbZ12,XbZ02)
...
lifexpect0.idmWeib <- function(s,a01,b01,a02,b02,a12,b12,bZ01=0,bZ02=0,bZ12=0) {
    ...
    ET01 = integrate(f=function(x) {
                         sapply(x,function(x) {integrate(f=function(y)
                             {
    S.weib(s,y,a01,b01,bZ01)*S.weib(s,y,a02,b02,bZ02)*iweibull(y,
                                                               a01,
                                                               b01,
                                                               bZ01)*S.weib(y,x,a12,b12,bZ12)}
                                                        ,lower=s,upper=x)$value})
                     },s,Inf)
    return(list(life.in.0.expectancy=ET0dot$value,life.expectancy.nondis=ET01$value+ET0dot$value,life.expectancy.dis=ET12$value))
