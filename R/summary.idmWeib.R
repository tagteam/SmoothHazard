summary.idmWeib <- function(object,conf.int=.95,digits=4,pvalDigits=4,eps=.0001, ...){

	if (!inherits(object,"idmWeib")) stop("Object must be of class 'idmWeib'")
	x <- object
	if (x$converged == 1){
		cat("illness using a parametrical Weibull hazard function.\n")
		cat("\n")
